...
Note: For the purposes of this post, I have trained a simple variational autoencoder on MNIST dataset .The code for all the results can be found in the following repository [Visualizing Latent Space in VAE]("{{ site.url }}/Notebooks/Variational_AutoEncoder_Implementation(PyTorch).ipynb") on my GitHub account.
